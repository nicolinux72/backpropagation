...
We developed the necessary formulas to finally calculate the derivative of \boldsymbol{each single component} of $\boldsymbol{a}$ with respect to each $a_m^{(l-1)}$ of the previous layer using the \eqref{eq:from-z} and the \ref{eq:prev}:
